...
public class FluttiePlugin implements FlutterPlugin, ActivityAware {
  ...
  private SparseArray<LottieComposition> loadedCompositions =
      new SparseArray<>();
  private AtomicInteger compositionRequestCounter = new AtomicInteger();

  private RenderingThreads renderingThreads;

  private FluttiePlugin(Registrar registrar) { this.registrar = registrar; }

  private void setUp() {
    final MethodChannel channel =
        new MethodChannel(registrar.messenger(), "fluttie/methods");

    channel.setMethodCallHandler(this);

    registrar.activity().getApplication().registerActivityLifecycleCallbacks(
        this);

    /*
     When multiple threads are working on rendering multiple animations
     simultaneously, at some we will inevitably send two texture updates to the
     flutter engine with a really short time delay in between. This, as it
     seems, will crash the engine :( There is an issue that could be related to
     this, https://github.com/flutter/flutter/issues/14169 that we're waiting on
     to be fixed. After that is fixed, we can enable multithreaded rendering,
     this code should otherwise work. If it still crashes, we can still take a
     look into that later on and report a new issue to the Flutter team.
     */
    renderingThreads = new RenderingThreads(1);
    renderingThreads.start();
  }

  Drawable.Callback getAnimationCallback() { return registrar.view(); }
  ...
  RenderingThreads getRenderingThreads() { return renderingThreads; }
  ...
  public void onMethodCall(MethodCall call, Result result) {
    ...
    case "prepareAnimation":
      ...
      int repeatMode = call.argument("repeat_reverse") ? ValueAnimator.REVERSE
      ...
      if (preferredWidth > 0 && preferredHeight > 0) {
        double scaleX =
            preferredWidth / (float)composition.getBounds().height();
        double scaleY =
            preferredHeight / (float)composition.getBounds().width();
      ...
      FluttieAnimation animation =
          new FluttieAnimation(this, texture, composition, scale, imagesPath);
  ...
  private void loadComposition(final int requestId, String sourceType,
                               String source, final Result result) {
    LottieListener<LottieComposition> listener = new LottieListener<
        LottieComposition>() {
      @Override
      public void onResult(@Nullable LottieComposition composition) {
        try {
          if (composition == null) {
            ...
            result.error("Could not load composition", "CompositionLoadError",
                         null);
            return;
          }
          ...
          if (!composition.getWarnings().isEmpty()) {
            Log.w(
                "FluttiePlugin",
                "You tried to load a composition that has some warnings, it might not be displayed correctly");
    ...
    case "asset":
      ...
      break;
    default:
      result.error("Unknown source type: " + sourceType, "UnknownSourceType",
                   null);
    }
  }
